...
namespace CorrePalabras.Controllers
{
    ...
    public class StoryLanguagesController : BaseController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] StoryLanguageRequest dto, [FromHeader] Guid userId)
        {
            try
            {
        ...
        [ProducesResponseType(typeof(ApiResponse<string>), 404)]
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(Guid id, [FromBody] StoryLanguageRequest dto, [FromHeader] Guid userId)
        {
            try 
            { 
