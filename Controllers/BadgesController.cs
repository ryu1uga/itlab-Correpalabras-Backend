...
namespace CorrePalabras.Controllers
{
    ...
    public class BadgesController : BaseController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> CreateBadge([FromBody] BadgeRequest dto, [FromHeader(Name = "UserId")] Guid userId)
        {
        ...
        [ProducesResponseType(typeof(ApiResponse<string>), 404)]
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateBadge(Guid id, [FromBody] BadgeRequest dto, [FromHeader(Name = "UserId")] Guid userId)
        {
            try
            {
